...
export function ExpenseAllocation() {
  ...
  const fetchAllocations = async () => {
    try {
      const data = await genericApi.list('expense_allocation');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingAllocation) {
        await genericApi.update('expense_allocation', editingId, formData);

        
      } else {
        await genericApi.create('expense_allocation', formData);
  ...
  const handleDelete = async (id: string, isLocked: boolean) => {
    ...
    try {
      await genericApi.delete('expense_allocation', id);
