...
export function TaxRates() {
  ...
  const fetchTaxRates = async () => {
    try {
      const data = await genericApi.list('tax_rates');
  ...
  const fetchCnssSettings = async () => {
    try {
      const data = await genericApi.list<CNSSSettings>('cnss_settings');
      if (data && data.length > 0) {
        const settings = data[0];
        setCnssSettings(settings);
        setCnssFormData({
          retirement_contribution: settings.retirement_contribution,
          amu_contribution: settings.amu_contribution,
          employer_deduction: settings.employer_deduction,
        });
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await genericApi.update('tax_rates', editingId, formData);
      } else {
  ...
  const handleCnssSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (cnssSettings) {
        await genericApi.update('cnss_settings', cnssSettings.id || (cnssSettings as any)._id, cnssFormData);
      } else {
