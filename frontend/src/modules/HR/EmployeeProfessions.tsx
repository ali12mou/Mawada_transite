...
export function EmployeeProfessions() {
  ...
  const fetchProfessions = async () => {
    try {
      const data = await genericApi.list('employee_professions');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await genericApi.update('employee_professions', editingId, formData);

        
      } else {
        await genericApi.create('employee_professions', formData);
