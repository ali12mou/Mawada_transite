...
export function CarrierMode() {
  ...
  const fetchModes = async () => {
    try {
      const data = await genericApi.list('carrier_modes');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingMode) {
        await genericApi.update('carrier_modes', editingId, formData);

        
      } else {
        await genericApi.create('carrier_modes', formData);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await genericApi.delete('carrier_modes', id);
