...
export function Routes() {
  ...
  const fetchRoutes = async () => {
    try {
      const data = await genericApi.list('routes');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRoute) {
        await genericApi.update('routes', editingId, formData);

        
      } else {
        await genericApi.create('routes', formData);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await genericApi.delete('routes', id);
