...
export function Carriers() {
  ...
  const fetchOptions = async () => {
    try {
      const [ownersData, modesData, routesData] = await Promise.all([
        genericApi.list('owners'),
        genericApi.list('carrier_modes'),
        genericApi.list('routes')
      ]);
  ...
  const fetchCarriers = async () => {
    try {
      const data = await genericApi.list<Carrier>('carriers');
      setCarriers(data || []);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await genericApi.update('carriers', editingId, payload);
      } else {
        await genericApi.create('carriers', {
          ...payload,
          created_by: user?.id,
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await genericApi.delete('carriers', id);
      fetchCarriers();
    } catch (error) {
  ...
  const getOwnerName = (id: string) => owners.find(o => o.id === id || o._id === id)?.name || '-';
  const getModeName = (id: string) => modes.find(m => m.id === id || m._id === id)?.name || '-';
  const getRouteInfo = (id: string) => {
    const r = routes.find(r => r.id === id || r._id === id);
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {currentItems.map((c) => (
                <tr key={c.id || (c as any)._id} className="hover:bg-[#0F3C66]/5 transition group">
                  ...
                  <td className="px-5 py-4 text-sm text-[#0F3C66] font-medium">{getOwnerName(c.owner_id)}</td>
                  <td className="px-5 py-4 text-sm text-gray-600">{getModeName(c.mode_id)}</td>
                  <td className="px-5 py-4 text-sm text-gray-600">
                    {getRouteInfo(c.route_id || '')}
                  </td>
                      ...
                      <ActionMenu
                        actions={[
                          ...
                          },
                          {
                            ...
                            icon: <Trash2 size={16} />,
                            onClick: () => handleDelete(c.id || (c as any)._id),
                            variant: 'danger',
                          },
                        ]}
                      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              >
                ...
                {modes.map(m => <option key={m.id || m._id} value={m.id || m._id}>{m.name}</option>)}
              </select>
            </div>
            <div className="md:col-span-2">
              ...
              >
                ...
                {routes.map(r => <option key={r.id || r._id} value={r.id || r._id}>{r.source} → {r.destination}</option>)}
              </select>
            </div>
          </div>
