...
export function ProductCategories() {
  ...
  const fetchCategories = async () => {
    try {
      const data = await genericApi.list('product_categories');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await genericApi.update('product_categories', editingId, formData);

        
      } else {
        await genericApi.create('product_categories', formData);
